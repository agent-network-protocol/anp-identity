...
3. The **本次演示身份** card shows the Handle from the confirmed creation request, the full DID, and the HTTP target. A missing Handle is shown explicitly, never inferred from the DID. Click **授权并发送签名请求** and approve the exact `POST /hello` request. The Host signs and dispatches it; the server should return **HTTP 200** with `verified: true`.
